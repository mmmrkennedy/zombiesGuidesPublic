...
function setupEventListeners() {
    ...
    if (input.hasAttribute('data-listeners-set')) {
        console.log('Event listeners already set up');
        return true;
    }

    // Filter elements on input with slight delay for better performance
    let searchTimeout;
    input.addEventListener('input', function () {
        console.log('Input event triggered, value:', input.value);
        clearTimeout(searchTimeout);
        searchTimeout = setTimeout(findElements, 200);
    });

    // Also trigger on keyup for immediate feedback
    input.addEventListener('keyup', function () {
        clearTimeout(searchTimeout);
        searchTimeout = setTimeout(findElements, 100);
    });

    // Mark as set up
    input.setAttribute('data-listeners-set', 'true');
...
document.addEventListener('DOMContentLoaded', function () {
    console.log('DOM loaded, attempting to set up event listeners');

    // Try immediately
    if (!setupEventListeners()) {
        // If elements not found, wait a bit for dynamic content to load
        console.log('Elements not found, waiting for dynamic content...');
        setTimeout(function () {
            if (!setupEventListeners()) {
...
window.addEventListener('load', function () {
    console.log('Window loaded, checking if listeners are set up');
    const input = document.getElementById('element-input');
    if (input && !input.hasAttribute('data-listeners-set')) {
        setupEventListeners();
        input.setAttribute('data-listeners-set', 'true');
    }
});